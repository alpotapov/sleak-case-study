...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const c of convs) {
      ...
      // Update conversation state to TRANSCRIPTION_QUEUED
      const { error: updErr } = await supabase
        .from('conversations')
        .update({ state: 'TRANSCRIPTION_QUEUED', updated_at: new Date().toISOString() })
        .eq('id', c.id);
    ...
    console.error('Error in enqueue-transcriptions', err);
